...
public class ExcelDemoDynamic {

	/**
	 * @param title
	 *            标题集合 tilte的长度应该与list中的model的属性个数一致
	 * @param maps
	 *            内容集合
	 * @param mergeIndex
	 *            合并单元格的列
	...
	public static String createExcel(
			String[] title,
			Map<String/* sheet名 */, List<Map<String/* 对应title的值 */, String>>> maps,
			int[] mergeIndex) {
		if (title.length == 0) {
			return null;
		}
...
//		Workbook workbook = new XSSFWorkbook();
		...
		for (Map.Entry<String, List<Map<String/* 对应title的值 */, String>>> entry : maps
				.entrySet()) {
				...
				while (iterator.hasNext()) {
					...
					for (int i = 0; i < title.length; i++) {
						...
						if (index > 1) {
							...
							old = poiModels.get(i) == null ? "" : poiModels
									.get(i).getContent();
						...
						for (int j = 0; j < mergeIndex.length; j++) {
							...
							if (index == 1) {
								 ...
								 * ||
								 * poiModels.get(i).getContent().equals(map.get
								 * (title[i])) && !poiModels.get(i -
								 * 1).getOldContent
								 * ().equals(map.get(title[i-1]))去掉就行
								 */

								if (!poiModels.get(i).getContent()
										.equals(map.get(title[i]))
										|| poiModels.get(i).getContent()
												.equals(map.get(title[i]))
										&& !poiModels.get(i - 1)
												.getOldContent()
												.equals(map.get(title[i - 1]))) {

									/* 当前行的当前列与上一行的当前列的内容不一致时，则把当前行以上的合并 */

									CellRangeAddress cra = new CellRangeAddress(
											poiModels.get(i).getRowIndex()/* 从第二行开始 */,
											index - 1/* 到第几行 */,
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 10; i++) {
			...
			HashMap<String, String> map = com.google.common.collect.Maps
					.newHashMap();
